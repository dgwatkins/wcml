jQuery(document).ready(function ($) {
    ...
    $(document).on('click', '#product_addons_data .js-wcml-dialog-trigger', function () {
        ...
        var default_price = $(this).parent().parent().find('input[type="number"]').val();
    ...
    $(document).on('change', '.wcml_custom_prices_input', function () {
        if ($(this).val() == 1) {
            $('#product_addons_data .js-wcml-dialog-trigger').each(function () {
                $(this).show();
            });
        } else {
            $('#product_addons_data .js-wcml-dialog-trigger').each(function () {
...
function maybe_set_prices_dialog_visible(element) {
    if (element.closest('#woocommerce-product-data').find('.wcml_custom_prices_input:checked').val() == 1) {
        element.closest('.wc-pao-addon').find('.wcml-option-prices .js-wcml-dialog-trigger').each(function () {
            jQuery(this).show();
        });
    }
}