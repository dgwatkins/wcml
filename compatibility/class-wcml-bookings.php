...
class WCML_Bookings {
	...
	public function sync_bookings( $original_product_id, $product_id, $language ) {
	    $find_all_booking_products = $this->get_translated_bookings( $original_product_id, false );
	    
	    foreach ( $find_all_booking_products as $find_all_booking_product ){
	     
		    $all_bookings_for_product = $this->wpdb->get_results( $this->wpdb->prepare( "SELECT post_id as id FROM {$this->wpdb->postmeta} WHERE meta_key = '_booking_product_id' AND meta_value = %d", $find_all_booking_product ) );
		    ...
		    foreach ( $all_bookings_for_product as $booking ) {
			    ...
			    if ( ! isset( $booking_translations[ $language ] ) ) {
				    ...
				    $this->update_translated_booking_meta( $booking_translations[ $language ], $find_all_booking_product, $language );
			    }
		    }
        }
	}
