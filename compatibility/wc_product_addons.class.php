...
class WCML_Product_Addons{
    ...
    function product_addons_filter($addons, $object_id){
        ...
        foreach($addons as $add_id => $addon){
            $addons[$add_id]['name'] = apply_filters( 'wpml_translate_single_string', 'wc_product_addons_strings', $object_id.'_addon_'.$addon['type'].'_'.$addon['position'].'_name', $addon['name']);
            $addons[$add_id]['description'] = apply_filters( 'wpml_translate_single_string', 'wc_product_addons_strings', $object_id.'_addon_'.$addon['type'].'_'.$addon['position'].'_description', $addon['description']);
            foreach($addon['options'] as $key=>$option){
                $addons[$add_id]['options'][$key]['label'] = apply_filters( 'wpml_translate_single_string', 'wc_product_addons_strings', $object_id.'_addon_'.$addon['type'].'_'.$addon['position'].'_option_label_'.$key, $option['label']);
