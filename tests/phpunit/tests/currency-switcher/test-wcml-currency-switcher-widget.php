...
class Test_WCML_Currency_Switcher_Widget extends OTGS_TestCase {
	 ...
	 * @test
	 */
	public function widget_with_sidebar_id_from_args() {
		$sidebar_id = 123;
		...
		$switcher = 'The currency switcher';

		\WP_Mock::onAction( 'wcml_currency_switcher' )
			->with( [ 'switcher_id' => $sidebar_id ] )
			->perform( function() use ( $switcher ) {
				echo $switcher;
			} );
		...
		$expected .= $widget_args['after_title'] . $switcher . $widget_args['after_widget'];
	 ...
	 * @test
	 */
	public function widget_with_sidebar_id_from_instance() {
		$sidebar_id = 123;
