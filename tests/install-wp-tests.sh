...
install_wp() {
  ...
  if [[ ! -d ${WP_CORE_DIR} || ${CI:0} == 1 ]]; then
    rm -rf ${WP_CORE_DIR}
    git clone ${GIT_CLONE_VERBOSITY_ARGS} --depth=1 https://github.com/WordPress/WordPress ${WP_CORE_DIR}
  fi
...
install_test_suite() {
  ...
  if [[ ! -d wordpress-develop || ${CI:0} == 1 ]]; then
    ...
    git clone ${GIT_CLONE_VERBOSITY_ARGS} --depth=1 https://github.com/WordPress/wordpress-develop
  fi
