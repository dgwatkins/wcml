	...
	<div class="wcml-section-content">
		<ul>
			<?php if ( defined( 'ICL_SITEPRESS_VERSION' ) ) : ?>
				<li>
					<i class="icon-ok"></i> <?php printf( __( '%s plugin is installed and active.', 'wpml-wcml' ), '<strong>WPML</strong>' ); ?>
				</li>
				<?php if ( $sitepress->setup() ): ?>
					<li>
						<i class="icon-ok"></i> <?php printf( __( '%s is set up.', 'wpml-wcml' ), '<strong>WPML</strong>' ); ?>
					</li>
				<?php else: ?>
					<li>
						<i class="icon-warning-sign"></i> <?php printf( __( '%s is not set up.', 'wpml-wcml' ), '<strong>WPML</strong>' ); ?>
					</li>
				<?php endif; ?>
			<?php endif; ?>
			<?php if ( defined( 'WPML_MEDIA_VERSION' ) ) : ?>
				<li>
					<i class="icon-ok"></i> <?php printf( __( '%s plugin is installed and active.', 'wpml-wcml' ), '<strong>WPML Media</strong>' ); ?>
				</li>
			<?php endif; ?>
			<?php if ( defined( 'WPML_TM_VERSION' ) ) : ?>
				<li>
					<i class="icon-ok"></i> <?php printf( __( '%s plugin is installed and active.', 'wpml-wcml' ), '<strong>WPML Translation Management</strong>' ); ?>
				</li>
			<?php endif; ?>
			<?php if ( defined( 'WPML_ST_VERSION' ) ) : ?>
				<li>
					<i class="icon-ok"></i> <?php printf( __( '%s plugin is installed and active.', 'wpml-wcml' ), '<strong>WPML String Translation</strong>' ); ?>
				</li>
			<?php endif; ?>
			<?php
			global $woocommerce;
			if ( class_exists( 'Woocommerce' ) ) : ?>
				<li>
					<i class="icon-ok"></i> <?php printf( __( '%s plugin is installed and active.', 'wpml-wcml' ), '<strong>WooCommerce</strong>' ); ?>
				</li>
			<?php endif; ?>
		</ul>
	</div>
		...
		<div class="wcml-section-content">
			...
			<?php if ( ! WPML_SUPPORT_STRINGS_IN_DIFF_LANG && $default_language != 'en' ): ?>
				...
				<?php if ( $sitepress_settings['st']['strings_language'] != 'en' ): ?>
					<p>
						<i class="icon-warning-sign"></i><strong><?php _e( 'Attention required: probable problem with URLs in different languages', 'wpml-wcml' ) ?></strong>
					</p>
				...
				<?php if ( $sitepress_settings['st']['strings_language'] == 'en' && empty( $woocommerce_wpml->settings['dismiss_non_default_language_warning'] ) ): ?>
					<p class="submit">
						<input id="wcml_dimiss_non_default_language_warning" type="button" class="button-primary"
						       value="<?php esc_attr_e( 'Dismiss', 'wpml-wcml' ) ?>"/>
				...
				<div class="wcml_miss_lang">
					...
					if ( isset( $miss_lang['lang'] ) ): ?>
						<p>
							...
							<input class="button" type="submit" name="create_pages"
					...
					<a id="wcmp_hide" class="wcmp_lang_hide"
					   href="javascript:void(0);"><?php _e( 'Hide this message', 'wpml-wcml' ) ?></a>
				</div>
				<p>
					<a id="wcmp_show" class="none"
					   href="javascript:void(0);"><?php _e( 'Show details about missing translations', 'wpml-wcml' ) ?></a>
				</p>
			</form>
		<?php else: ?>
			<p>
	...
	<div class="wcml-section-content js-tax-translation">
		...
		<ul>
			...
			<?php if ( $no_tax_to_update ): ?>
				<li>
					<i class="icon-ok"></i> <?php _e( 'Right now, there are no taxonomy terms needing translation.', 'wpml-wcml' ); ?>
				</li>
			<?php endif; ?>
		</ul>
