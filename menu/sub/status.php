		...
		<div class="wcml-section-content">
			<ul class="wcml-status-list">
				...
				<?php if ( ! empty( $miss_slug_lang ) ): ?>
					...
					<li>
						<i class="otgs-ico-warning"></i> <?php _e( "Your product permalink base is not translated to:", 'wpml-wcml' ); ?>
						...
						<?php _e( "The urls for the translated products will not work.", 'wpml-wcml' ); ?>
						<a class="button-secondary"
						   href="<?php echo admin_url( 'admin.php?page=' . WPML_ST_FOLDER . '/menu/string-translation.php&search=' . $prod_slug . '&context=WordPress&em=1' ); ?>">
							<?php _e( "Translate URLs", 'wpml-wcml' ); ?>
						</a>
					</li>
				<?php endif; ?>
				...
				<?php if ( ! WPML_SUPPORT_STRINGS_IN_DIFF_LANG && $default_language != 'en' ): ?>
					<?php if ( $sitepress_settings['st']['strings_language'] != 'en' ): ?>
						<li>
							<i class="otgs-ico-warning"></i> <?php _e( "Your site's default language is not English and the strings language is also not English.", 'wpml-wcml' ) ?>
							<small>
								<i class="otgs-ico-help"></i>
								<a href="http://wpml.org/?page_id=355545">
									<?php _e( "Running WooCommerce multilingual with default language other than English.", 'wpml-wcml' ); ?>
								</a>
							</small>
							<p>
								<?php _e( "This may cause problems with URLs in different languages.", 'wpml-wcml' ) ?>
								...
								<button id="wcml_fix_strings_language" type="button" class="button-secondary">
									<?php esc_attr_e( "Change default language", 'wpml-wcml' ) ?>
								</button>
							</p>
					</li>
					<?php elseif ( empty( $woocommerce_wpml->settings['dismiss_non_default_language_warning'] ) ): ?>
						<li>
							<i class="otgs-ico-warning"></i> <?php _e( "Your site's default language is not English.", 'wpml-wcml' ) ?>
							<small>
								<i class="otgs-ico-help"></i>
								<a href="http://wpml.org/?page_id=355545">
									<?php _e( "Running WooCommerce multilingual with default language other than English.", 'wpml-wcml' ); ?>
							...
							<p>
								<?php _e( "There are some settings that require careful attention.", 'wpml-wcml' ) ?>
							</p>
						</li>
					<?php endif; ?>
...
<div class="wcml-section">
	<div class="wcml-section-header">
		<h3>
			...
			<i class="otgs-ico-help wcml-tip"
			   data-tip="<?php _e( 'To run a multilingual e-commerce site, you need to have the WooCommerce shop pages translated to all the site\'s languages. Once all the pages are installed you can add the translations for them from this menu.', 'wpml-wcml' ) ?>"></i>
		</h3>
	</div>
	<div class="wcml-section-content">
				...
				<form method="post" action="<?php echo $_SERVER["REQUEST_URI"]; ?>">
					...
					<li>
						...
						<button class="button-secondary aligncenter" type="submit" name="create_pages">
							<?php _e( 'Create missing translations', 'wpml-wcml' ); ?>
						</button>
						<?php /*
				...
				<li class="js-tax-translation-<?php echo $taxonomy ?>">
					<?php if ( $untranslated = WCML_Terms::get_untranslated_terms_number( $taxonomy ) ): ?>
						...
						<?php else: ?>
							...
							<?php printf( __( '%d %s are missing translations.', 'wpml-wcml' ), $untranslated, get_taxonomy( $taxonomy )->labels->name ); ?>
							<a class="button-secondary"
							   href="<?php echo admin_url( 'admin.php?page=wpml-wcml&tab=' . $taxonomy ) ?>">
								<?php printf( __( 'Translate %s', 'wpml-wcml' ), get_taxonomy( $taxonomy )->labels->name ); ?>
							</a>
							<small class="actions">
								<a href="#ignore-<?php echo $taxonomy ?>"
								   title="<?php esc_attr_e( 'This taxonomy does not require translation.', 'wpml-wcml' ) ?>"><?php _e( 'Exclude from translation', 'wpml-wcml' ) ?></a>
							</small>
						<?php endif; ?>
					<?php else: ?>
