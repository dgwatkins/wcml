...
class Hooks implements \IWPML_Backend_Action, \IWPML_REST_Action, \IWPML_DIC_Action {
	...
	public function addJoin( $clauses ) {
		$clauses[] = "LEFT JOIN {$this->wpdb->postmeta} wcml_language_postmeta
			ON wcml_language_postmeta.post_id = IF(
				{$this->wpdb->prefix}wc_order_stats.parent_id = 0,
				{$this->wpdb->prefix}wc_order_stats.order_id,
				{$this->wpdb->prefix}wc_order_stats.parent_id
			)
			AND wcml_language_postmeta.meta_key = 'wpml_language'";
