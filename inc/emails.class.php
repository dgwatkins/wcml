...
class WCML_Emails{
    ...
    function email_heading_completed( $order_id, $no_checking = false ){
        ...
        if(class_exists('WC_Email_Customer_Completed_Order') || $no_checking){
            ...
            if($heading)
                $woocommerce->mailer()->emails['WC_Email_Customer_Completed_Order']->heading = apply_filters( 'wpml_translate_single_string', $heading[0]->context,'[woocommerce_customer_completed_order_settings]heading',$heading[0]->value);
            $subject = $this->wcml_get_email_string_info( '[woocommerce_customer_completed_order_settings]subject' );
            if($subject)
                $woocommerce->mailer()->emails['WC_Email_Customer_Completed_Order']->subject = apply_filters( 'wpml_translate_single_string', $subject[0]->context,'[woocommerce_customer_completed_order_settings]subject',$subject[0]->value);
            ...
            if($heading_downloadable)
                $woocommerce->mailer()->emails['WC_Email_Customer_Completed_Order']->heading_downloadable = apply_filters( 'wpml_translate_single_string', $heading_downloadable[0]->context,'[woocommerce_customer_completed_order_settings]heading_downloadable',$heading_downloadable[0]->value);
            ...
            if($subject_downloadable)
                $woocommerce->mailer()->emails['WC_Email_Customer_Completed_Order']->subject_downloadable = apply_filters( 'wpml_translate_single_string', $subject_downloadable[0]->context,'[woocommerce_customer_completed_order_settings]subject_downloadable',$subject_downloadable[0]->value);
    ...
    function email_heading_processing($order_id){
        ...
        if(class_exists('WC_Email_Customer_Processing_Order')){
            ...
            if($heading)
                $woocommerce->mailer()->emails['WC_Email_Customer_Processing_Order']->heading = apply_filters( 'wpml_translate_single_string', $heading[0]->context,'[woocommerce_customer_processing_order_settings]heading',$heading[0]->value);
            $subject = $this->wcml_get_email_string_info( '[woocommerce_customer_processing_order_settings]subject' );
            if($subject)
                $woocommerce->mailer()->emails['WC_Email_Customer_Processing_Order']->subject = apply_filters( 'wpml_translate_single_string', $subject[0]->context,'[woocommerce_customer_processing_order_settings]subject',$subject[0]->value);
    ...
    function email_heading_note($args){
        ...
        if(class_exists('WC_Email_Customer_Note')){
            ...
            if($heading)
                $woocommerce->mailer()->emails['WC_Email_Customer_Note']->heading = apply_filters( 'wpml_translate_single_string', $heading[0]->context,'[woocommerce_customer_note_settings]heading',$heading[0]->value);

            $subject = $this->wcml_get_email_string_info( '[woocommerce_customer_note_settings]subject' );
            if($subject)
                $woocommerce->mailer()->emails['WC_Email_Customer_Note']->subject = apply_filters( 'wpml_translate_single_string', $subject[0]->context,'[woocommerce_customer_note_settings]subject',$subject[0]->value);
    ...
    function admin_email($order_id){
        ...
        if(class_exists('WC_Email_New_Order')){
            ...
            foreach($recipients as $recipient){
                ...
                if($heading)
                    $woocommerce->mailer()->emails['WC_Email_New_Order']->heading = apply_filters( 'wpml_translate_single_string', $heading[0]->context,'[woocommerce_new_order_settings]heading',$heading[0]->value);
                $subject = $this->wcml_get_email_string_info( '[woocommerce_new_order_settings]subject' );
                if($subject)
                    $woocommerce->mailer()->emails['WC_Email_New_Order']->subject = apply_filters( 'wpml_translate_single_string', $subject[0]->context,'[woocommerce_new_order_settings]subject',$subject[0]->value);
