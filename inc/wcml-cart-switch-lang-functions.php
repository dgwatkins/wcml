...
class WCML_Cart_Switch_Lang_Functions{
    ...
    function wcml_language_switch_dialog( ){
        ...
        if( $this->lang_from && $this->lang_to && $request_url && ( isset( WC()->cart ) && !empty( WC()->cart->get_cart_for_session() ) ) ) {
            ...
            $confirmation_message = sprintf( __( 'You are switched language and there are items in the cart. If you stay in %s language, the cart will be emptied and you will need to add the items again to the cart.', 'woocommerce-multilingual' ), $new_language_details[ 'display_name' ] );
