    ...
    function wcml_language_switch_dialog( ){
        ...
        if( $dependencies->check() ){
            ...
            if( $this->lang_from && $this->lang_to && $request_url && !empty( $cart_for_session ) ) {
                ...
                $confirmation_message = sprintf( esc_html__( "You've switched language and there are items in the cart. If you keep the %s language, the cart will be emptied and you will have to add the items again to the cart.", 'woocommerce-multilingual' ), $new_language_details[ 'display_name' ] );
